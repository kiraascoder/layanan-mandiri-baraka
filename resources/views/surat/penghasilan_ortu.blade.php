...
@section('title', 'Surat Ket Penghasilan Orang Tua')

@section('content')
    <div class="container mx-auto mt-4 p-6 bg-[#123524] rounded-lg shadow-lg max-w-4xl">
        <h2 class="text-2xl font-semibold mb-6 text-[#EFE3C2]">Surat Keterangan Penghasilan Orang Tua</h2>
        {{-- Alert Error --}}
        @if ($errors->any())
        ...
        <form action="{{ route('citizen.buat-surat.submit') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="jenis_surat" value="penghasilan_ortu">
            ...
            <div id="form_izin_usaha">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm text-[#EFE3C2]">Penghasilan Orang Tua Per Bulan</label>
                        <input type="text" name="data_surat[penghasilan_ortu]" id="penghasilan_ortu"
                            class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="Masukkan Penghasilan Orang Tua" required>
                    </div>
                </div>
            </div>
